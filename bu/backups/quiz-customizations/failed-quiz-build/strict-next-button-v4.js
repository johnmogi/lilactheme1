/**
 * Strict Next Button Controller v4 - Fixed Version for LearnDash Quiz
 * 
 * This script ensures proper quiz flow in force hint mode:
 * 1. User must view hint first
 * 2. User must select an answer after viewing hint
...
(function() {
    ...
    // State tracking
    const hintViewedMap = {};
    const checkedAfterHintMap = {};  // Important for tracking if user has selected an answer AFTER seeing hint
    ...
    function setupEventListeners(settings) {
        ...
        document.addEventListener('hintShown', function(event) {
            ...
            if (questionId) {
                ...
                // Mark as needing answer selection in force hint mode
                if (settings.forceHintMode) {
                    // Reset checked after hint to false - they need to select again
                    checkedAfterHintMap[questionId] = false;
            ...
            if (event.target.classList.contains('wpProQuiz_questionInput')) {
                ...
                if (questionItem) {
                    ...
                    if (questionId && hintViewedMap[questionId]) {
                        // If hint was viewed, mark this question as checked after hint
                        checkedAfterHintMap[questionId] = true;
                        debug('Answer selected after hint for question', questionId);
    ...
    function handleHintButtonClick(questionItem, settings) {
        ...
        // Reset the checked after hint status - they need to select again
        checkedAfterHintMap[questionId] = false;
    ...
    function handleCheckButtonClick(questionItem, settings) {
        ...
        if (hintViewedMap[questionId]) {
            checkedAfterHintMap[questionId] = true;
            debug('Answer checked after viewing hint');
        }
    ...
    function isAnswerCorrect(questionItem) {
        ...
        if (inputs.length > 0) {
            ...
            return false; // Don't automatically consider it correct
        }
    ...
    function updateNextButton(questionItem, settings) {
        ...
        if (settings.forceHintMode) {
            if (viewedHint && hasSelection) {
                // In force hint mode, if hint is viewed AND user has made a selection, enable next button
                enableButton(nextButton);
                debug('Enabling next button - hint viewed and answer selected in force hint mode');
            } else {
                ...
                if (!viewedHint) {
                    ...
                    debug('Disabling next button - no answer selected after viewing hint');
                }
            }
        } 
        // Normal mode logic
    ...
    function revealQuizAnswers() {
        ...
        questionItems.forEach((item, index) => {
            ...
            answerHTML += `<div class="question-debug">
                ...
                <p>Hint viewed: ${hintViewedMap[questionId] ? 'YES' : 'NO'}</p>
                <p>Selection after hint: ${checkedAfterHintMap[questionId] ? 'YES' : 'NO'}</p>`;
            ...
            answerItems.forEach((answer, aIndex) => {
                ...
                const isSelected = answer.querySelector('.wpProQuiz_questionInput:checked') !== null;
                const answerText = answer.textContent.trim();
                
                answerHTML += `<li class="${isCorrect ? 'correct-answer' : ''} ${isSelected ? 'selected-answer' : ''}">
