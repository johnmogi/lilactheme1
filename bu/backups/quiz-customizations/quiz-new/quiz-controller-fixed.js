...
(function() {
    ...
    function checkAnswerCorrect(questionItem, questionId) {
        ...
        for (let i = 0; i < selectedItems.length; i++) {
            ...
            if (listItem) {
                // Check for various class indicators
                if (listItem.classList.contains('wpProQuiz_answerCorrect') || 
                    ...
                    log(`Question ${questionId} is correct (detected by class on answer item)`);
                    return true;
                }
                ...
                if (listItem.dataset.correct === '1' || 
                    ...
                    log(`Question ${questionId} is correct (detected by data attribute)`);
                    return true;
                }
                
                // Check input attributes directly
                const input = listItem.querySelector('input');
                if (input && (input.dataset.correct === 'true' || input.getAttribute('data-correct') === '1')) {
                    log(`Question ${questionId} is correct (detected by input attribute)`);
                    return true;
                }
            }
        }
        
        // Method 3: Check for correct response message
